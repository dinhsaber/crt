...
def batch_recover_from_share_folder():
    ...
    if df.empty:
        print("Không tìm thấy share hợp lệ nào cho các image_id đã chọn!")
        return
    ...
    for image_id in selected_ids:
        ...
        if not os.path.exists(meta_path):
            print(f"Không tìm thấy meta cho image_id {image_id}, bỏ qua!")
            continue
        ...
        print(df_img)
        if len(df_img) < k:
            print(f"Không đủ {k} share hợp lệ để khôi phục image_id {image_id}!")
            continue
    ...
    while True:
        ...
        if choice == "1":
            ...
            if not os.path.exists(folder) or not os.path.isdir(folder):
                ...
                continue
            try:
                k = int(input(f"Nhập ngưỡng k (số client tối thiểu để khôi phục, k <= {N_CLIENTS}): ").strip())
